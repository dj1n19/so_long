...
/*   Updated: 2022/10/11 16:34:17 by bgenie           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	ft_key_down(int keycode, t_datas *datas)
{
	...
	else if (datas->pt_running == 0 && datas->mt_running == 0 && keycode == LETTER_E)
	{
		datas->keycode = keycode;
		datas->pt_running = 1;
		pthread_create(datas->player_thread, datas->attr, ft_player_attack, datas);
	}
	else if (datas->pt_running == 0 && datas->mt_running == 0 && keycode == NUM_1)
	{
		datas->keycode = keycode;
		datas->pt_running = 1;
...
int	ft_hook(t_datas *datas)
{
	...
	if (datas->map->map[datas->player->y / TILESIZE][datas->player->x / TILESIZE] == 'E' && datas->map->is_portal_open)
		ft_close(datas);
	draw(datas);
	usleep(25000);
}
